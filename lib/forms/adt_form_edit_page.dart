...
class ADTeditWorkingDocFormPage extends StatelessWidget {
  ...
  const ADTeditWorkingDocFormPage({
    super.key,
    this.initialRows,
    this.onSave,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Articulated Dump Truck Inspection Edit Page'),
...
class _WorkingDocFormState extends State<WorkingDocForm> {
  ...
  bool _isUploading = false; // show loading animation while uploading
  ...
  Widget build(BuildContext context) {
          ...
          children: [
            Row(
              children: [
                ...
                // Use the same unit list as in adt_form_page.dart
                _labeledField(
                  "Pilih Unit",
                  DropdownButtonFormField<String>(
    ...
    List<DataRow> _buildTableRows() {
      ...
      // Group 1
      addDivider("▶ Dump Vesel");
      for (final r in rows.where((r) => r.number >= 1 && r.number <= 5)) {
        tableRows.add(_buildRow(r));
      }

      // Group 2
      addDivider("▶ Hydrolic Dump");
      for (final r in rows.where((r) => r.number >= 6 && r.number <= 13)) {
        tableRows.add(_buildRow(r));
      }
      ...
      // Group 5
      addDivider("▶ Engine");
      for (final r in rows.where((r) => r.number >= 23 && r.number <= 34)) {
        tableRows.add(_buildRow(r));
      }

      // Group 6
      addDivider("▶ Transmission and Hub Wheel");
      for (final r in rows.where((r) => r.number >= 35 && r.number <= 40)) {
        tableRows.add(_buildRow(r));
      }

      // Group 6
      addDivider("▶ Steering");
      for (final r in rows.where((r) => r.number >= 41 && r.number <= 43)) {
        tableRows.add(_buildRow(r));
      }

      // Group 7
      addDivider("▶ Brake");
      for (final r in rows.where((r) => r.number >= 44 && r.number <= 49)) {
        tableRows.add(_buildRow(r));
      }

      // Group 8
      addDivider("▶ Cabin");
      for (final r in rows.where((r) => r.number >= 50 && r.number <= 58)) {
        tableRows.add(_buildRow(r));
      }

      // Group 9
      addDivider("▶ Electrical");
      for (final r in rows.where((r) => r.number >= 59 && r.number <= 66)) {
        tableRows.add(_buildRow(r));
      }

      // Group 9
      addDivider("▶ Fuel / Oil / Lube / Tire");
      for (final r in rows.where((r) => r.number >= 67 && r.number <= 70)) {
        tableRows.add(_buildRow(r));
      }
                    ...
                    onPressed: () async {
                      ...
                      try {
                        ...
                        final exportFileName =
                            '${username}_adt_form_${inspectionDate}_${inspectionTime}.xlsx';
                        ...
                        if (topForm['foto_extra'] != null &&
                          ...
                          if (await extraPhotoFile.exists()) {
                            final ext =
                                ...
                                : '';
                            extraPhotoFilename =
                                'adt_form_${inspectionDate}_${inspectionTime}_topform$ext';
                          ...
                          if (row.photo?.path != null && row.photo!.path.isNotEmpty) {
                            ...
                            if (await photoFile.exists()) {
                              final ext =
                                  ...
                                  : '';
                              photoFilename =
                                  'adt_form_${inspectionDate}_${inspectionTime}_row${row.number}$ext';
                        ...
                        if (excelData != null) {
                          ...
                          final zipFile = File(
                            '$dirPath/${username}_adt_form_${inspectionDate}_${inspectionTime}.zip',
                          );
                          ...
                          if (response.statusCode == 200) {
                            ...
                            try {
                              final box = Hive.box('forms');
                              final hiveKey = context
                                  .findAncestorWidgetOfExactType<
                                    ADTeditWorkingDocFormPage
                                  >()
                                  ?.hiveKey;
                            ...
                            // stop loading BEFORE showing success notification
                            setState(() {
                              _isUploading = false;
                            });
                            ...
                            // stop loading BEFORE showing failure notification
                            setState(() {
                              _isUploading = false;
                            });
                            ScaffoldMessenger.of(context).showSnackBar(
                          ...
                          // show excel exported info after upload; ensure loading stopped
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
