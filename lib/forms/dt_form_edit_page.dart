...
class DTFormEditPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Dump Truck Inspection Edit Page'),
...
class _WorkingDocFormState extends State<WorkingDocForm> {
  ...
  Widget build(BuildContext context) {
          ...
          children: [
            Row(
              children: [
                ...
                // Use the same unit list as in dt_form_page.dart
                _labeledField(
                  "Pilih Unit",
                  DropdownButtonFormField<String>(
    ...
    List<DataRow> _buildTableRows() {
      ...
      for (final r in rows.where((r) => r.number >= 0 && r.number <= 5)) {
        tableRows.add(_buildRow(r));
      }

      // Group 2
      addDivider("▶ Hydraulic System");
      for (final r in rows.where((r) => r.number >= 6 && r.number <= 14)) {
        tableRows.add(_buildRow(r));
      }

      // Group 3
      addDivider("▶ Cylinder and Hose");
      for (final r in rows.where((r) => r.number >= 15 && r.number <= 16)) {
        tableRows.add(_buildRow(r));
      }

      // Group 4
      addDivider("▶ Frame / Chasis");
      for (final r in rows.where((r) => r.number >= 17 && r.number <= 25)) {
        tableRows.add(_buildRow(r));
      }
      ...
      // Group 9
      addDivider("▶ Cabin");
      for (final r in rows.where((r) => r.number >= 53 && r.number <= 61)) {
        tableRows.add(_buildRow(r));
      }

      // Group 10
      addDivider("▶ Electrical");
      for (final r in rows.where((r) => r.number >= 62 && r.number <= 69)) {
        tableRows.add(_buildRow(r));
      }

      // Group 11
      addDivider("▶ Fuel / Oil / Lube / Tire");
      for (final r in rows.where((r) => r.number >= 70 && r.number <= 73)) {
        tableRows.add(_buildRow(r));
      }
                    ...
                    onPressed: () async {
                      ...
                      try {
                        ...
                        final exportFileName =
                            '${username}_dt_form_${inspectionDate}_${inspectionTime}.xlsx';
                        ...
                        if (topForm['foto_extra'] != null &&
                          ...
                          if (await extraPhotoFile.exists()) {
                            final ext =
                                ...
                                : '';
                            extraPhotoFilename =
                                'dt_form_${inspectionDate}_${inspectionTime}_topform$ext';
                          ...
                          if (row.photo?.path != null && row.photo!.path.isNotEmpty) {
                            ...
                            if (await photoFile.exists()) {
                              final ext =
                                  ...
                                  : '';
                              photoFilename =
                                  'dt_form_${inspectionDate}_${inspectionTime}_row${row.number}$ext';
                        ...
                        if (excelData != null) {
                          ...
                          final zipFile = File(
                            '$dirPath/${username}_dt_form_${inspectionDate}_${inspectionTime}.zip',
                          );
                          ...
                          if (response.statusCode == 200) {
                            ...
                            try {
                              final box = Hive.box('dt_forms');
                              final hiveKey = context
                                  .findAncestorWidgetOfExactType<DTFormEditPage>()
                                  ?.hiveKey;
