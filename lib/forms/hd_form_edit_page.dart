...
class HDFormEditPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('High Dump Inspection Edit Page'),
...
class _WorkingDocFormState extends State<WorkingDocForm> {
  ...
  Widget build(BuildContext context) {
          ...
          children: [
            Row(
              children: [
                ...
                // Use the same unit list as in hd_form_page.dart
                _labeledField(
                  "Pilih Unit",
                  DropdownButtonFormField<String>(
    ...
    List<DataRow> _buildTableRows() {
      ...
      for (final r in rows.where((r) => r.number >= 4 && r.number <= 14)) {
        tableRows.add(_buildRow(r));
      }
      ...
      for (final r in rows.where((r) => r.number >= 17 && r.number <= 23)) {
        tableRows.add(_buildRow(r));
      }

      // Group 5
      addDivider("▶ Engine");
      for (final r in rows.where((r) => r.number >= 24 && r.number <= 38)) {
        tableRows.add(_buildRow(r));
      }

      // Group 6
      addDivider("▶ Transmisi dan Hub Wheel");
      for (final r in rows.where((r) => r.number >= 39 && r.number <= 44)) {
        tableRows.add(_buildRow(r));
      }

      // Group 7
      addDivider("▶ Steering");
      for (final r in rows.where((r) => r.number >= 45 && r.number <= 47)) {
        tableRows.add(_buildRow(r));
      }

      // Group 8
      addDivider("▶ Brake");
      for (final r in rows.where((r) => r.number >= 48 && r.number <= 53)) {
        tableRows.add(_buildRow(r));
      }

      // Group 9
      addDivider("▶ Cabin");
      for (final r in rows.where((r) => r.number >= 54 && r.number <= 63)) {
        tableRows.add(_buildRow(r));
      }

      // Group 10
      addDivider("▶ Electrical");
      for (final r in rows.where((r) => r.number >= 64 && r.number <= 74)) {
        tableRows.add(_buildRow(r));
      }
                    ...
                    onPressed: () async {
                      ...
                      try {
                        ...
                        final exportFileName =
                            '${username}_hd_form_${inspectionDate}_${inspectionTime}.xlsx';
                        ...
                        if (topForm['foto_extra'] != null &&
                          ...
                          if (await extraPhotoFile.exists()) {
                            final ext =
                                ...
                                : '';
                            extraPhotoFilename =
                                'hd_form_${inspectionDate}_${inspectionTime}_topform$ext';
                          ...
                          if (row.photo?.path != null && row.photo!.path.isNotEmpty) {
                            ...
                            if (await photoFile.exists()) {
                              final ext =
                                  ...
                                  : '';
                              photoFilename =
                                  'hd_form_${inspectionDate}_${inspectionTime}_row${row.number}$ext';
                        ...
                        if (excelData != null) {
                          ...
                          final zipFile = File(
                            '$dirPath/${username}_hd_form_${inspectionDate}_${inspectionTime}.zip',
                          );
                          ...
                          if (response.statusCode == 200) {
                            ...
                            try {
                              final box = Hive.box('hd_forms');
                              final hiveKey = context
                                  .findAncestorWidgetOfExactType<HDFormEditPage>()
                                  ?.hiveKey;
