...
class WLFormPage extends StatelessWidget {
  final List<InspectionRow>? initialRows;
  final void Function(List<InspectionRow> rows)? onSave;

  const WLFormPage({super.key, this.initialRows, this.onSave});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Wheel Loader Inspection Form'),
...
class _WorkingDocFormState extends State<WorkingDocForm> {
  ...
  List<InspectionRow> _defaultRows() => [
    ...
    InspectionRow(
      number: 1,
      inspectionRef: 'Tread',
      thing: 'Periksa Kedalaman Tread Tyre dan Kondisi Tread',
      condition: 'good',
      notes: "",
      action: 'none',
      actionDate: null,
    ...
    InspectionRow(
      number: 2,
      inspectionRef: 'Side Wall',
      thing: 'Periksa Kondisi Sidewall Tyre',
      condition: 'good',
      notes: "",
      action: 'none',
      actionDate: null,
    ...
    InspectionRow(
      number: 3,
      inspectionRef: 'Tyre Pressure',
      thing: 'Periksa Pressure Tyre',
      condition: 'good',
      notes: "",
      action: 'none',
      actionDate: null,
    ...
    InspectionRow(
      number: 4,
      inspectionRef: 'Usia Tyre',
      thing: 'Periksa Usia Tyre',
      condition: 'good',
      notes: "",
      action: 'none',
      actionDate: null,
    ...
    InspectionRow(
      number: 5,
      inspectionRef: 'Wear Bucket Protection',
      thing: 'Periksa Kondisi Bucket Protector',
      condition: 'good',
      notes: "",
      action: 'none',
      actionDate: null,
    ...
    InspectionRow(
      number: 6,
      inspectionRef: 'Cutting Edge',
      thing: 'Periksa Keausan Cutting Edge',
      condition: 'good',
      notes: "",
      action: 'none',
      actionDate: null,
    ...
    InspectionRow(
      number: 7,
      inspectionRef: 'Pin/Bushing Bucket LH',
      thing: 'Periksa Keausan',
      condition: 'good',
      notes: "",
      action: 'none',
      actionDate: null,
    ...
    InspectionRow(
      number: 8,
      inspectionRef: 'Pin/Bushing Bucket RH',
      thing: 'Periksa Keausan',
      condition: 'good',
      notes: "",
      action: 'none',
    ...
    InspectionRow(
      number: 9,
      inspectionRef: 'Cyl Tilt Bucket',
      thing: 'Periksa Kebocoran Pada Cylinder',
      condition: 'good',
      notes: "",
      action: 'none',
      actionDate: null,
    ...
    InspectionRow(
      number: 12,
      inspectionRef: 'Pin/Bushing Tilt Cyl Bucket',
    ...
    InspectionRow(
      number: 13,
      inspectionRef: 'Pin/Bushing Steering RH',
      thing: 'Periksa pin/bushing dan grease',
      condition: 'good',
      notes: "",
      action: 'none',
    ...
    InspectionRow(
      number: 14,
      inspectionRef: 'Pin/Bushing Steering LH',
      thing: 'Periksa pin/bushing dan grease',
      condition: 'good',
      notes: "",
      action: 'none',
    ...
    InspectionRow(
      number: 20,
      inspectionRef: 'Axle Housing Front',
      thing: 'Periksa Keretakan Pada Housing',
      condition: 'good',
      notes: "",
      action: 'none',
      actionDate: null,
    ...
    InspectionRow(
      number: 21,
      inspectionRef: 'Axle Housing Rear',
      thing: 'Periksa Keretakan Pada Housing',
      condition: 'good',
      notes: "",
      action: 'none',
    ...
    InspectionRow(
      number: 22,
      inspectionRef: 'Rocker Boom Frame',
      thing: 'Periksa Deformasi & Keretakan',
      condition: 'good',
      notes: "",
      action: 'none',
      actionDate: null,
  ...
  Widget build(BuildContext context) {
    ...
    List<DataRow> _buildTableRows() {
      ...
      // Group 1
      addDivider("▶ Tyre");
      for (final r in rows.where((r) => r.number >= 0 && r.number <= 4)) {
        tableRows.add(_buildRow(r));
      }

      // Group 2
      addDivider("▶ Bucket");
      for (final r in rows.where((r) => r.number >= 5 && r.number <= 8)) {
        tableRows.add(_buildRow(r));
      }

      // Group 3
      addDivider("▶ Cylinder and Hose");
      for (final r in rows.where((r) => r.number >= 9 && r.number <= 14)) {
        tableRows.add(_buildRow(r));
      }

      // Group 4
      addDivider("▶ Hydraulic System");
      for (final r in rows.where((r) => r.number >= 15 && r.number <= 19)) {
        tableRows.add(_buildRow(r));
      }

      // Group 5
      addDivider("▶ Frame / Chasis");
      for (final r in rows.where((r) => r.number >= 20 && r.number <= 25)) {
        tableRows.add(_buildRow(r));
      }
      ...
      for (final r in rows.where((r) => r.number >= 38 && r.number <= 41)) {
        tableRows.add(_buildRow(r));
      }

      // Group 8
      addDivider("▶ Cabin");
      for (final r in rows.where((r) => r.number >= 42 && r.number <= 50)) {
        tableRows.add(_buildRow(r));
      }

      // Group 9
      addDivider("▶ Electrical");
      for (final r in rows.where((r) => r.number >= 51 && r.number <= 58)) {
        tableRows.add(_buildRow(r));
      }

      // Group 10
      addDivider("▶ Fuel / Oil / Lube");
      for (final r in rows.where((r) => r.number >= 59 && r.number <= 62)) {
        tableRows.add(_buildRow(r));
      }
