...
// import 'package:inspection_app_gmt_3/forms/hd_form_edit_page.dart';
// import 'package:inspection_app_gmt_3/forms/hd_form_page.dart';

// class HDListPage extends StatelessWidget {
//   const HDListPage({Key? key}) : super(key: key);

//   Future<Box> _openFormsBox() async {
//     if (!Hive.isBoxOpen('hd_forms')) {
//       if (!Hive.isAdapterRegistered(0)) {
//         // quick check for initialization
//         await Hive.initFlutter();
//       }
//       return await Hive.openBox('hd_forms');
//     }
//     return Hive.box('hd_forms');
//   }
...
//         return Scaffold(
//           appBar: AppBar(title: const Text("HD Forms")),
//           body: ValueListenableBuilder(
//             valueListenable: formsBox.listenable(),
//             builder: (context, Box box, _) {
//               final allForms = box.values.toList();
//               final hdForms = allForms.where((form) {
//                 final f = form as Map;
//                 return f["form"]?["form_type"] == "hd_form";
//               }).toList();

//               if (hdForms.isEmpty) {
//                 return const Center(child: Text("No HD forms found."));
//               }

//               return ListView.builder(
//                 itemCount: hdForms.length,
//                 itemBuilder: (context, index) {
//                   final form = hdForms[index] as Map;
...
//                             context,
//                             MaterialPageRoute(
//                               builder: (context) => HDFormEditPage(
...
//             heroTag: 'hd_list_fab_main',
//             onPressed: () {
//               Navigator.of(context).push(
//                 MaterialPageRoute(builder: (context) => const HDFormPage()),
//               );
//             },
...
class HDListPage extends StatefulWidget {
  const HDListPage({Key? key}) : super(key: key);

  @override
  State<HDListPage> createState() => _HDListPageState();
}

class _HDListPageState extends State<HDListPage> {
  String _searchQuery = "";
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.of(
            context,
          ).push(MaterialPageRoute(builder: (context) => const HDFormPage()));
        },
        child: const Icon(Icons.add),
      ),
    );
  ...
  Widget _buildBodyContent(Box formsBox) {
    return ValueListenableBuilder(
      ...
      builder: (context, Box box, _) {
        ...
        final HDFormsWithKeys = [];
        for (int i = 0; i < allForms.length; i++) {
          final form = allForms[i] as Map;
          if (form["form"]?["form_type"] == "hd_form") {
            HDFormsWithKeys.add({'key': allKeys[i], 'form': form});
          }
        }

        final filteredForms = HDFormsWithKeys.where((item) {
        ...
        if (filteredForms.isEmpty) {
          return const Center(child: Text("No matching HD forms found."));
        }
                ...
                child: ListTile(
                  ...
                  onTap: () {
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (context) =>
                            HDFormEditPage(formJson: form, hiveKey: hiveKey),
                      ),
                    );
                  },
                ),
