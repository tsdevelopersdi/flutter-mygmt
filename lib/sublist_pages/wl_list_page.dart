...
// import 'package:inspection_app_gmt_3/forms/wl_form_edit_page.dart';
// import 'package:inspection_app_gmt_3/forms/wl_form_page.dart';

// class WLListPage extends StatelessWidget {
//   const WLListPage({Key? key}) : super(key: key);

//   Future<Box> _openFormsBox() async {
//     if (!Hive.isBoxOpen('wl_forms')) {
//       if (!Hive.isAdapterRegistered(0)) {
//         // quick check for initialization
//         await Hive.initFlutter();
//       }
//       return await Hive.openBox('wl_forms');
//     }
//     return Hive.box('wl_forms');
//   }
...
//         return Scaffold(
//           appBar: AppBar(title: const Text("WL Forms")),
//           body: ValueListenableBuilder(
//             valueListenable: formsBox.listenable(),
//             builder: (context, Box box, _) {
//               final allForms = box.values.toList();
//               final wlForms = allForms.where((form) {
//                 final f = form as Map;
//                 return f["form"]?["form_type"] == "wl_form";
//               }).toList();

//               if (wlForms.isEmpty) {
//                 return const Center(child: Text("No WL forms found."));
//               }

//               return ListView.builder(
//                 itemCount: wlForms.length,
//                 itemBuilder: (context, index) {
//                   final form = wlForms[index] as Map;
...
//                             context,
//                             MaterialPageRoute(
//                               builder: (context) => WLFormEditPage(
...
//             heroTag: 'wl_list_fab_main',
//             onPressed: () {
//               Navigator.of(context).push(
//                 MaterialPageRoute(builder: (context) => const WLFormPage()),
//               );
//             },
...
class WLListPage extends StatefulWidget {
  const WLListPage({Key? key}) : super(key: key);

  @override
  State<WLListPage> createState() => _WLListPageState();
}

class _WLListPageState extends State<WLListPage> {
  String _searchQuery = "";
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.of(
            context,
          ).push(MaterialPageRoute(builder: (context) => const WLFormPage()));
        },
        child: const Icon(Icons.add),
      ),
    );
  ...
  Widget _buildBodyContent(Box formsBox) {
    return ValueListenableBuilder(
      ...
      builder: (context, Box box, _) {
        ...
        final WLFormsWithKeys = [];
        for (int i = 0; i < allForms.length; i++) {
          final form = allForms[i] as Map;
          if (form["form"]?["form_type"] == "wl_form") {
            WLFormsWithKeys.add({'key': allKeys[i], 'form': form});
          }
        }

        final filteredForms = WLFormsWithKeys.where((item) {
        ...
        if (filteredForms.isEmpty) {
          return const Center(child: Text("No matching WL forms found."));
        }
                ...
                child: ListTile(
                  ...
                  onTap: () {
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (context) =>
                            WLFormEditPage(formJson: form, hiveKey: hiveKey),
                      ),
                    );
                  },
                ),
