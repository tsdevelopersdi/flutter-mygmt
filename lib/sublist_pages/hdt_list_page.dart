...
// import 'package:inspection_app_gmt_3/forms/hdt_form_edit_page.dart';
// import 'package:inspection_app_gmt_3/forms/hdt_form_page.dart';

// class HDTListPage extends StatelessWidget {
//   const HDTListPage({Key? key}) : super(key: key);

//   Future<Box> _openFormsBox() async {
//     if (!Hive.isBoxOpen('hdt_forms')) {
//       if (!Hive.isAdapterRegistered(0)) {
//         // quick check for initialization
//         await Hive.initFlutter();
//       }
//       return await Hive.openBox('hdt_forms');
//     }
//     return Hive.box('hdt_forms');
//   }
...
//         return Scaffold(
//           appBar: AppBar(title: const Text("HDT Forms")),
//           body: ValueListenableBuilder(
//             valueListenable: formsBox.listenable(),
//             builder: (context, Box box, _) {
//               final allForms = box.values.toList();
//               final hdtForms = allForms.where((form) {
//                 final f = form as Map;
//                 return f["form"]?["form_type"] == "hdt_form";
//               }).toList();

//               if (hdtForms.isEmpty) {
//                 return const Center(child: Text("No HDT forms found."));
//               }

//               return ListView.builder(
//                 itemCount: hdtForms.length,
//                 itemBuilder: (context, index) {
//                   final form = hdtForms[index] as Map;
...
//                             context,
//                             MaterialPageRoute(
//                               builder: (context) => HDTFormEditPage(
...
//             heroTag: 'hdt_list_fab_main',
//             onPressed: () {
//               Navigator.of(context).push(
//                 MaterialPageRoute(builder: (context) => const HDTFormPage()),
//               );
//             },
...
class HDTListPage extends StatefulWidget {
  const HDTListPage({Key? key}) : super(key: key);

  @override
  State<HDTListPage> createState() => _HDTListPageState();
}

class _HDTListPageState extends State<HDTListPage> {
  String _searchQuery = "";
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.of(
            context,
          ).push(MaterialPageRoute(builder: (context) => const HDTFormPage()));
        },
        child: const Icon(Icons.add),
      ),
    );
  ...
  Widget _buildBodyContent(Box formsBox) {
    return ValueListenableBuilder(
      ...
      builder: (context, Box box, _) {
        ...
        final HDTFormsWithKeys = [];
        for (int i = 0; i < allForms.length; i++) {
          final form = allForms[i] as Map;
          if (form["form"]?["form_type"] == "hdt_form") {
            HDTFormsWithKeys.add({'key': allKeys[i], 'form': form});
          }
        }

        final filteredForms = HDTFormsWithKeys.where((item) {
        ...
        if (filteredForms.isEmpty) {
          return const Center(child: Text("No matching HDT forms found."));
        }
                ...
                child: ListTile(
                  ...
                  onTap: () {
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (context) =>
                            HDTFormEditPage(formJson: form, hiveKey: hiveKey),
                      ),
                    );
                  },
                ),
