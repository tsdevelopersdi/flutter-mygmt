...
// import 'package:inspection_app_gmt_3/forms/dt_form_edit_page.dart';
// import 'package:inspection_app_gmt_3/forms/dt_form_page.dart';

// class DTListPage extends StatelessWidget {
//   const DTListPage({Key? key}) : super(key: key);

//   Future<Box> _openFormsBox() async {
//     if (!Hive.isBoxOpen('dt_forms')) {
//       if (!Hive.isAdapterRegistered(0)) {
//         // quick check for initialization
//         await Hive.initFlutter();
//       }
//       return await Hive.openBox('dt_forms');
//     }
//     return Hive.box('dt_forms');
//   }
...
//         return Scaffold(
//           appBar: AppBar(title: const Text("DT Forms")),
//           body: ValueListenableBuilder(
//             valueListenable: formsBox.listenable(),
//             builder: (context, Box box, _) {
//               final allForms = box.values.toList();
//               final dtForms = allForms.where((form) {
//                 final f = form as Map;
//                 return f["form"]?["form_type"] == "dt_form";
//               }).toList();

//               if (dtForms.isEmpty) {
//                 return const Center(child: Text("No DT forms found."));
//               }

//               return ListView.builder(
//                 itemCount: dtForms.length,
//                 itemBuilder: (context, index) {
//                   final form = dtForms[index] as Map;
...
//                             context,
//                             MaterialPageRoute(
//                               builder: (context) => DTFormEditPage(
...
//             heroTag: 'dt_list_fab_main',
//             onPressed: () {
//               Navigator.of(context).push(
//                 MaterialPageRoute(builder: (context) => const DTFormPage()),
//               );
//             },
...
class DTListPage extends StatefulWidget {
  const DTListPage({Key? key}) : super(key: key);

  @override
  State<DTListPage> createState() => _DTListPageState();
}

class _DTListPageState extends State<DTListPage> {
  String _searchQuery = "";
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.of(
            context,
          ).push(MaterialPageRoute(builder: (context) => const DTFormPage()));
        },
        child: const Icon(Icons.add),
      ),
    );
  ...
  Widget _buildBodyContent(Box formsBox) {
    return ValueListenableBuilder(
      ...
      builder: (context, Box box, _) {
        ...
        final DTFormsWithKeys = [];
        for (int i = 0; i < allForms.length; i++) {
          final form = allForms[i] as Map;
          if (form["form"]?["form_type"] == "dt_form") {
            DTFormsWithKeys.add({'key': allKeys[i], 'form': form});
          }
        }

        final filteredForms = DTFormsWithKeys.where((item) {
        ...
        if (filteredForms.isEmpty) {
          return const Center(child: Text("No matching DT forms found."));
        }
                ...
                child: ListTile(
                  ...
                  onTap: () {
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (context) =>
                            DTFormEditPage(formJson: form, hiveKey: hiveKey),
                      ),
                    );
                  },
                ),
